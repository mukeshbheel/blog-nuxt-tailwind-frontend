    ...
    <div class="sm:main sm:basis-3/4" v-if="allPostsData">
      <div class="sm:flex flex-row justify-center" v-for="item in pageBlogs" :key="item._id">
        <div class="allPosts my-9">
          ...
          <div class="readMore flex flex-row justify-center">
            <button class="bg-gray-800 text-white px-5 py-5" @click="$router.push(`/BlogDetails/${item._id}`)">Read
              More</button>
          </div>
        </div>
      </div>
      <div class="pagination" v-if="allPostsData && allPostsData.length > 0">
        <button
          ...
          v-for="i in allPostsData.length%3==0?allPostsData.length/3:Math.ceil(allPostsData.length/3)" :key="i"
          @click="blogPage=i; $router.push(`/?page=${i}`)">{{i}}</button>
      </div>
    </div>
